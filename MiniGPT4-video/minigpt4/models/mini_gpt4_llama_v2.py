...
from minigpt4.models import models_vit
...
class MiniGPT4_llama_v2(Blip2Base):
    ...
    def __init__(
        ...
        #fyy add 
        if need_speech:
            self.speech_encoder = models_vit.__dict__['vit_base_patch16'](
                num_classes=527,
                drop_path_rate=0.1,
                global_pool=True,
                mask_2d=True,
            ...
            if True:
                ...
                in_chans=1
                emb_dim = 768
                self.speech_encoder.patch_embed = PatchEmbed_new(img_size=img_size, patch_size=(16,16), in_chans=1, embed_dim=emb_dim, stride=16) # no overlap. stride=img_size=16
                num_patches = self.speech_encoder.patch_embed.num_patches
                #num_patches = 512 # assume audioset, 1024//16=64, 128//16=8, 512=64x8
                self.speech_encoder.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, emb_dim), requires_grad=False)  # fixed sin-cos embedding
                    
            if finetune:
                checkpoint = torch.load(finetune, map_location='cpu')
                print("Load pre-trained checkpoint from: %s" % finetune)
                checkpoint_model = checkpoint['model']
                state_dict = self.speech_encoder.state_dict()

                # load pre-trained model
                msg = self.speech_encoder.load_state_dict(checkpoint_model, strict=False)
                print(msg)
            #######################维度乱写的
            ...
            #     param.requires_grad = False
            # self.speech_proj = self.speech_proj.eval()
            # self.speech_proj.train = disabled_train
        print('Loading LLAMA')
        ...
        print("1. Basic model dtype:", next(self.llama_model.parameters()).dtype)
        ...
        # ckptt = torch.load('/home/develop/fyy/MiniGPT4-video-main/ego-sentiment/weights/1video_llama_checkpoint_cleaned.pth', map_location="cpu")
        # msg = self.llama_model.load_state_dict(ckptt['model'], strict=False)
        # print("2. After applying PEFT dtype:", next(self.llama_model.parameters()).dtype)
        # # # #######################
        # # ckpt = torch.load(ckpt_path, map_location="cpu")
        # #     msg = model.load_state_dict("/home/develop/fyy/MiniGPT4-video-main/ego-sentiment/weights/1video_llama_checkpoint_changed.pth", strict=False)
        # # lora_model=PeftModel.from_pretrained(self.llama_model,
        # #                                      "/home/develop/fyy/MiniGPT4-video-main/ego-sentiment/weights/1video_llama_checkpoint_changed.pth",torch_dtype=torch.float16)
        # mmodel = self.llama_model.merge_and_unload()
        # print("3. After merge_and_unload dtype:", next(mmodel.parameters()).dtype)
        # mmodel.save_pretrained("/home/develop/fyy/MiniGPT4-video-main/new_merge")
        # self.llama_tokenizer.save_pretrained("/home/develop/fyy/MiniGPT4-video-main/new_merge")
        
        # loaded_model = LlamaForCausalLM.from_pretrained("/home/develop/fyy/MiniGPT4-video-main/new_merge", torch_dtype=torch.float16)

        # # 检查 dtype - 从磁盘加载模型后
        # print("4. After loading from disk dtype:", next(loaded_model.parameters()).dtype)
        # print("++++++++++++++++++++++")
    ...
    def encode_speech(self, speech):
        ...
        with self.maybe_autocast():
            speech = self.speech_encoder(speech).to(device)
            speech_proj = self.speech_proj(speech)
            return speech_proj
        
    def get_context_emb(self, prompt, img_list, speech_list=None):
        img_device = img_list[0].device
        prompt_segs = prompt.split('<ImageHere>')
        assert len(prompt_segs) == len(img_list) + 1, "Unmatched numbers of image placeholders and images."
        seg_embs = []
        if speech_list != None:
            pp=prompt_segs[0].split('<SpeechHere>')
            pre_tokens = self.llama_tokenizer(pp[0], return_tensors="pt", add_special_tokens=True).to(img_device)
            pre_embed = self.embed_tokens(pre_tokens.input_ids)
            pre = torch.cat([pre_embed, speech_list], dim=1)
            prompt_segs[0] = pp[1]
    
        for i, seg in enumerate(prompt_segs):
            tmp = self.embed_tokens(self.llama_tokenizer(
                seg, return_tensors="pt", add_special_tokens=(i==0 and speech_list is None)).to(img_device).input_ids)
            if i==0 and speech_list!=None:
                tmp = torch.cat([pre,tmp], dim=1)
            seg_embs.append(tmp)
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompts, lengths=None, speech_embeds=None):
        if prompts is None or len(prompts) == 0:
            ...
            for idx, (each_img_embed, each_prompt) in enumerate(zip(img_embeds, prompts)):
                ...
                interleave_emb = []
                if speech_embeds != None:
                    pp=p_segs[0].split('<SpeechHere>')
                    pre_tokens = self.llama_tokenizer(pp[0], return_tensors="pt", add_special_tokens=False).to(img_embeds.device)
                    pre_embed = self.embed_tokens(pre_tokens.input_ids)
                    interleave_emb.append(torch.cat([pre_embed, speech_embeds[idx][None][None]], dim=1))
                    p_segs[0] = pp[1]
    ...
    def preparing_embedding(self, samples):
        ...
        #########################################fyy add
        if 'speech' in samples:
            speech_embeds = self.encode_speech(samples["speech"])
        else:
        ...
        if 'conv_q' in samples:
            ...
            if 'length' in samples:
                ...
                cond_embeds, cond_atts = self.prompt_wrap(img_embeds, img_atts, instruction, samples['length'], speech_embeds)
            else:
    ...
    def generate(
        ...
        if speech != None:
            speech_lists = []
            speech_embeds = self.encode_speech(speech.to(self.device))
            for i in range(speech_embeds.shape[0]):
                speech_lists.append(speech_embeds[0][None][None])
        if lengths is not None:
            image_lists = []
        ...
        if speech != None:
            batch_embs = [self.get_context_emb(text, img_list, speech_list) for text, img_list, speech_list in zip(texts, image_lists, speech_lists)]
        else:
    ...
    def from_config(cls, cfg):
        ...
        need_speech = cfg.get("need_speech",False)
        ...
        if ckpt_path:
            ...
            # ckpt1=torch.load("/home/develop/fyy/MiniGPT4-video-main/ego-sentiment/weights/1video_llama_checkpoint_changed.pth",map_location="cpu")
            # msg1=model.load_state_dict(ckpt1['model'], strict=False)
            ckpt2 = torch.load(ckpt_path, map_location="cpu")
            msg2 = model.load_state_dict(ckpt2['model'], strict=False)
