...
def generate_subtitles(video_path,existed_subtitles):
    ...
    audio_path = f"/home/develop/fyy/MiniGPT4-video-main/tmp_fyy_data/mp3/{video_id}"+'.mp3'
    os.makedirs("/home/develop/fyy/MiniGPT4-video-main/tmp_fyy_data/mp3",exist_ok=True)
    if existed_subtitles.get(video_id,False):
        print("subtitle already generated")
        return f"/home/develop/fyy/MiniGPT4-video-main/tmp_fyy_data/subtitles_vtt/{video_id}"+'.vtt'
    try:
        extract_audio(video_path,audio_path)
        print("successfully extracted")
        os.system(f"/home/develop/anaconda3/envs/minigpt4/bin/whisper {audio_path}  --language English --model large --output_format vtt --output_dir /home/develop/fyy/MiniGPT4-video-main/tmp_fyy_data/subtitles_vtt")
...
class VideoChatGPTDataset(BaseDataset, __DisplMixin):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths,subtitles_path,model_name='llama2',add_subtitles=False,
                 need_speech = False, audio_conf=None, wav_base=None,dataset=None):
        """
        vis_root (string): Root directory of images (e.g. coco/images/)
        ann_root (string): directory to store the annotation file
        """
        ...
        self.videos_has_subtitles = {}
        self.subtitle_folder = subtitles_path
        if self.add_subtitles and dataset!="ego-sentiment":
            for sub in os.listdir(self.subtitle_folder):
                video_id = sub.split('.')[0]
                self.videos_has_subtitles[video_id] = True
    ...
    def __getitem__(self, index):
        ...
        img_placeholder = ""
        # has_subtitles = self.videos_has_subtitles.get(video_id, False)
        if self.dataset=='ego-sentiment':
            # print(1)
            video_path = os.path.join(self.vis_root,video_id.split('-')[0],f"{video_id.split('-')[1]}.mp4")
            if self.add_subtitles:
                has_subtitles = os.path.exists(os.path.join(self.subtitle_folder,video_id.split('-')[0],f"{video_id.split('-')[1]}.vtt"))
        else:
            has_subtitles = self.videos_has_subtitles.get(video_id, False)
            video_path = os.path.join(self.vis_root,f'{video_id}.avi')   
        if self.add_subtitles and has_subtitles:
            if self.dataset=='ego-sentiment':
                # print(1)
                subtitle_path = os.path.join(self.subtitle_folder,video_id.split('-')[0],f"{video_id.split('-')[1]}.vtt")
            else:
                subtitle_path = os.path.join(self.subtitle_folder, f'{video_id}.vtt')
            # Load the VTT subtitle file
            # print(subtitle_path)
            ...
            has_subtitles=False
        # if self.add_subtitles and not has_subtitles:
        ...
        previous_sub = ""
        if self.need_speech:
            img_placeholder += '<Speech><SpeechHere>'
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
                break
        ...
        if self.need_speech:
            if self.dataset=='ego-sentiment':
                wav_path = self.base_wav+'/'+ video_id.split('-')[0]+'/'+video_id.split('-')[1]+'.wav'
            else:
                wav_path = os.path.join(self.base_wav, f'{video_id}.wav')
            fbank, mix_lambda = self._wav2fbank(wav_path)
...
class VideoChatGPTEvalDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.dataset=='ego-sentiment':
            # print(1)
            video_path = os.path.join(self.videos_path,video_id.split('-')[0],f"{video_id.split('-')[1]}.mp4")
            if self.add_subtitles:
                has_subtitles = os.path.exists(os.path.join(self.subtitle_folder,video_id.split('-')[0],f"{video_id.split('-')[1]}.vtt"))
        else:
            has_subtitles = self.videos_has_subtitles.get(video_id, False)
            video_path = os.path.join(self.videos_path,f'{video_id}.avi')   
        ...
        ##################fyy add
        if self.need_speech:
            return images,instruction,answer,self.length,video_id,fbank.unsqueeze(0)
        else:
            return images,instruction,answer,self.length,video_id,torch.tensor([1])
...
class VideoChatGPTEval_consistancy(torch.utils.data.Dataset):
    def __init__(self, vis_processor, videos_path, ann_path,subtitles_path,annotations_keys,add_subtitles=True,llm_name="llama2",
                 need_speech = False, audio_conf=None, wav_base=None):
        if llm_name=="llama2":
            self.length = 45
            self.max_sub_len = 400
        else:
